
# Projeto Contraturno

Este repositório contém algoritmos desenvolvidos utilizando o **Portugol Web**, uma plataforma online que permite escrever, executar e compartilhar códigos em Portugol diretamente pelo navegador — sem precisar instalar nenhum programa.

O Portugol Web é ideal para estudar lógica de programação, testar algoritmos e desenvolver atividades acadêmicas de forma simples e acessível.

---

## O que é o Portugol Web?

O **Portugol Web** é uma IDE online criada para interpretar algoritmos escritos em Portugol.
Ele oferece:

* Ambiente de programação direto no navegador
* Execução imediata do código
* Interface simples e intuitiva
* Compartilhamento rápido dos algoritmos
* Suporte à sintaxe tradicional de Portugol (similar ao Portugol Studio)

---

##  Como usar o Portugol Web

### 1. Abra o site do Portugol Web

Acesse o ambiente online (basta procurar por **"Portugol Web IDE"** no Google).
Não é necessário baixar nada.

---

### 2. Crie um novo algoritmo

Ao abrir a IDE, você verá um editor com a estrutura básica:

```portugol
programa
{
    funcao inicio()
    {
        // seu código aqui
    }
}
```

---

### 3. Escreva seu código

Exemplo simples:

```portugol
programa
{
    funcao inicio()
    {
        cadeia nome

        escreva("Digite seu nome: ")
        leia(nome)

        escreva("Olá, ", nome, "!")
    }
}
```

---

### 4. Execute o algoritmo

Clique em **“Executar”** ou pressione **Ctrl + Enter**.
A saída aparecerá no painel lateral ou inferior, dependendo da versão da IDE.

---

## Conteúdo deste repositório

* Tipos de dados em Portugol
* Exemplos de lógica de programação em portugol
* Estruturas condicionais e de repetição em portugol
* Algoritmos com funções, vetores e matrizes em portugol

---
## Membros e suas funções
* Brayan Ferrari - Lider de Projeto
* Arthur Horácio - Programador
* Kainat Fleck - Analista de Requisitos
* Maria Eduarda Marka - Comunicadora
* Yasmin Takeda - Designer

---
## Orientador
* Andrei Doberstein
* Valentinar Aguiar Cardozo

